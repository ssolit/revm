...
impl L1BlockInfo {
    ...
    pub fn try_fetch<DB: Database>(
        ...
        if !spec_id.is_enabled_in(OpSpecId::ECOTONE) {
            let l1_fee_overhead = db.storage(L1_BLOCK_CONTRACT, L1_OVERHEAD_SLOT)?.into();
            let l1_fee_scalar = db.storage(L1_BLOCK_CONTRACT, L1_SCALAR_SLOT)?.into();
            ...
            let l1_blob_base_fee = db.storage(L1_BLOCK_CONTRACT, ECOTONE_L1_BLOB_BASE_FEE_SLOT)?.word;
            ...
            let l1_fee_overhead = empty_ecotone_scalars
                ...
                .transpose()?;
            let l1_fee_overhead = l1_fee_overhead.map(|x| x.into());
